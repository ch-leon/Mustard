...
///
/// `project` appears in two forms across the app — the KB folder name the config
/// actually stores (`"DL-Knowledge-Base"`) and the short code the maps are keyed by
/// (`"DL"`). This reconciles them to the area name (`"Digital Licence"`) so routing
/// (bridge export, board area-stamping) works regardless of which form is held.
